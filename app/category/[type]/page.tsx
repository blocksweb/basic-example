...
import { Star, ChevronDown, ChevronUp, Filter, SlidersHorizontal, Heart, ShoppingCart, Search, X } from "lucide-react";
...
export default function CategoryPage({ params }: { params: { type: string } }) {
  ...
  const products = mockProducts.filter(product => product.category === type);
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
              <h1 className="text-3xl font-montserrat font-bold text-primary">
                {categoryTitle === "Casual" ? "Casual Collectie" : "Sportswear Collectie"}
              </h1>
              <p className="text-gray-600 mt-1">
                {categoryTitle === "Casual" 
                  ? "Everyday apparel for style and comfort" 
                  : "Performance clothing for your active life"}
              </p>
            </div>
            ...
            <div className="flex mt-4 md:mt-0 space-x-2 bg-white rounded-md border p-1">
              <Link href="/category/casual">
                <Button variant={type === "casual" ? "default" : "ghost"} className={type === "casual" ? "bg-primary" : ""}>
                  Casual
                </Button>
              </Link>
              <Link href="/category/sportswear">
                <Button variant={type === "sportswear" ? "default" : "ghost"} className={type === "sportswear" ? "bg-accent" : ""}>
                  Sportswear
                </Button>
              </Link>
            </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mt-6">
            ...
            <div className="flex flex-wrap gap-2 mb-4 md:mb-0">
              {activeFilters.length > 0 ? (
                ...
                <span className="text-sm text-gray-500">32 items</span>
              )}
            </div>
          ...
          <div className={`md:w-1/4 lg:w-1/5 ${isMobileFiltersOpen ? 'block' : 'hidden'} md:block`}>
            <div className="sticky top-4 bg-white rounded-lg border p-4">
              ...
              <div className="mb-4 border-b pb-4">
                ...
                >
                  <span>Gender</span>
                  {openFilterSections.gender ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                </button>
                {openFilterSections.gender && (
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <Checkbox id="gender-men" />
                      <label htmlFor="gender-men" className="text-sm">Heren</label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Checkbox id="gender-women" />
                      <label htmlFor="gender-women" className="text-sm">Dames</label>
                    </div>
                  </div>
                )}
              </div>
